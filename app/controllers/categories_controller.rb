class CategoriesController < ApplicationController
  def index
    ...
    @categories = Category.visible.where(parent_id: nil).includes(:children)
    ...
    if params[:category_id].present?
      begin
        ...
        @skus = @current_category.all_descendant_skus.active.includes(:category, images_attachments: :blob).page(params[:page]).per(12)
      ...
      if @kind.present?
        @skus = Sku.joins(:category).where(categories: { category_kind: @kind, hidden: false }).active.includes(:category, images_attachments: :blob).order(position: :asc, created_at: :desc).page(params[:page]).per(12)
      else
        @skus = Sku.joins(:category).where(categories: { hidden: false }).active.includes(:category, images_attachments: :blob).order(position: :asc, created_at: :desc).page(params[:page]).per(12)
      end
    end
  end
