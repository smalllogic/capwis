class CategoriesController < ApplicationController
  def index
    ...
    if params[:category_id].present?
      ...
      if @kind.present?
        @skus = Sku.joins(:category).where(categories: { category_kind: @kind, hidden: false }, status: 'active').includes(:category, images_attachments: :blob).order(position: :desc, created_at: :desc).page(params[:page]).per(20)
      else
        @skus = Sku.joins(:category).where(categories: { hidden: false }, status: 'active').includes(:category, images_attachments: :blob).order(position: :desc, created_at: :desc).page(params[:page]).per(20)
      end
    end
  end
